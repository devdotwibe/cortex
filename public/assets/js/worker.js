decryptData=async function(encryptedData, key) { 
    ...
    let keyBuffer =new TextEncoder().encode(key) // new Uint8Array(Array.prototype.map.call(key, function(c) { return c.charCodeAt(0); }));// new TextEncoder().encode(key);
    ...
    let decryptedData =await  crypto.subtle.decrypt({ name: 'AES-CBC',length: 256, iv: ivBuffer }, keyArray, encryptedDataBuffer).then(function(d){
        console.log(d)
        return d;
    }).catch(function(re){
        console.log(re)
