decryptData=async function(encryptedData, key) { 
    ...
    let decryptedData =await  crypto.subtle.decrypt({ name: 'AES-CBC',length: 256, iv: ivBuffer }, new TextDecoder().encode(decryptedData), encryptedDataBuffer).then(function(d){
        console.log(d)
        return d;
    }).catch(function(re){
        console.log(re)
