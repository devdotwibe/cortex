        ...
        <li class="side-item {{ request()->is('full-mock-exam') ? 'active' : '' }}">
            <a href="{{ route('full-mock-exam.index') }}">Full Mock Exam</a>
                ...
                <li class="side-item {{request()->is('live-class*') ? 'active':''}}">
                        ...
                        <a @if(auth('admin')->check() &&!(auth('web')->user()->is_free_access) && (optional(auth('web')->user()->subscription())->status ?? "") !== "subscribed") data-bs-toggle="modal" data-bs-target="#adminsubModal" href="{{ route('live-class.index') }}"  @else href="{{ route('live-class.index') }}" @endif >  
                        <span class="side-icon" >
