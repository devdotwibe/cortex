            ...
            <div class="exam-progress-inner">
                <div class="exam-progress-inner-item exam-left">
                    <div class="progress-main">
                        ...
                        <div class="exam-exit ">
                            <a href="{{ route('topic-test.index') }}" title="Exit" data-title="Exit" aria-label="Exit"
                                data-toggle="tooltip">
        ...
        function loadlessonreview(reviewurl) {
            $.get(reviewurl || "{{ route('topic-test.preview', $userExamReview->slug) }}", function(res) {
                ...
                $.each(res.data, function(k, v) {
                    ...
                    `).fadeIn();
                    $.get("{{ route('topic-test.preview', $userExamReview->slug) }}", {
                        question: v.slug
                    }, function(ans) {
                            ...
                            $(`#mcq-${lesseonId}-list-progress`).append(`
                                <div class="form-progress-ans ans-${av.iscorrect?"select":"no-select"}"> 
                                    <div class="form-progress">       
                                        ...
                                        <progress id="user-answer-${lesseonId}-ans-progress-item-${ai}" max="100" value="${av.total_user_answered||0}"/><span>${((av.total_user_answered||0)*1).toFixed(2)}%</span>
                                    </div>  
                                </div>
                            `)
                            if (av.iscorrect) {
