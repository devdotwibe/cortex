    ...
    <section class="header_nav">
        <div class="header_wrapp">
            <div class="header_title">
                ...
                <h2><span>{{ $exam->subtitle($category->id, 'Module ' . ($category->getIdx() + 1)) }}</span><span> :
                    </span><span>{{ $category->name }}</span></h2>
            </div>
        </div>
    </section>
                                ...
                                <div class="col-md-6">
                                    @if ($user->is_free_access || (optional($user->subscription())->status ?? '') == 'subscribed' || $k == 0)
                                        <a
                                            @if (
                                                $user->progress(
                                                    'exam-' . $exam->id . '-module-' . $category->id . '-lesson-' . $item->id . '-complete-review',
                                                    'no') == 'yes') @elseif (
                                                $user->progress('exam-' . $exam->id . '-module-' . $category->id . '-lesson-' . $item->id . '-complete-date', '') ==
                                                    '')
                                    @guest('admin')   href="{{ route('learn.lesson.show', ['category' => $category->slug, 'sub_category' => $item->slug]) }}" @endguest
                                @else
                                    href="#" onclick="loadlessonreviews('{{ route('learn.lesson.history', ['category' => $category->slug, 'sub_category' => $item->slug]) }}', {{ $k + 1 }}); return false;" @endif>
                                        @else
                                    ...
                                    <div class="lesson-row">
                                        <div class="lesson-row-title">
                                            ...
                                            <h3>: {{ $item->name }} </h3>
                                            <h4>{{ round($user->progress('exam-' . $exam->id . '-module-' . $category->id . '-lesson-' . $item->id, 0), 2) }}%
                                            </h4>
                                        </div>
