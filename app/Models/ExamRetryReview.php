...
class ExamRetryReview extends Model
{
    ...
    public function avgMark(){ 
        ...
        switch ($this->name) {
            case 'full-mock-exam':
                $anscnt = ExamRetryAnswer::where('exam_retry_review_id','<=',$userExamReviewId)->where('exam_id',$this->exam_id)->whereIn('exam_retry_review_id',ExamRetryReview::where('name','full-mock-exam')->where('id','<=',$userExamReviewId)->groupBy('user_id')->select(DB::raw('MAX(id)')))->where('iscorrect',true)->where('user_answer',true)->count();
                $exmcnt = ExamRetryReview::where('id','<=',$userExamReviewId)->where('exam_id',$this->exam_id)->whereIn('id',ExamRetryReview::where('name','full-mock-exam')->where('id','<=',$userExamReviewId)->groupBy('user_id')->select(DB::raw('MAX(id)')))->count();
                break; 
            case 'topic-test':
                $anscnt = ExamRetryAnswer::where('exam_retry_review_id','<=',$userExamReviewId)->where('exam_id',$this->exam_id)->whereIn('exam_retry_review_id',ExamRetryReview::where('name','topic-test')->where('category_id',$categoryId)->where('id','<=',$userExamReviewId)->groupBy('user_id')->select(DB::raw('MAX(id)')))->where('iscorrect',true)->where('user_answer',true)->count();
