...
class PrivateClassHomeWorkController extends Controller
{
    ...
    public function booklethistory(Request $request, HomeWork $homeWork, HomeWorkBook $homeWorkBook)
    {
        ...
        return DataTables::of(HomeWorkReview::where('user_id', $user->id)->where('home_work_id', $homeWork->id)->where('home_work_book_id', $homeWorkBook->id)->select('id','user_id','slug', 'created_at', 'progress','updated_at'))
