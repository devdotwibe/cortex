...
class TopicExamController extends Controller
{
    ...
    public function retrysubmit(Request $request,  UserExamReview $userExamReview)
    {
        if (session("exam-retry-" . $userExamReview->id)) {
            ...
            return redirect()->route('user.topic-test.retry.result', ['user_exam_review'=>$userExamReview->id,'exam_retry_review'=>$review->slug])->with("success", "Topic Test Submited")->with("review", $review->id);
        }
